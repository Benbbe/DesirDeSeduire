...
{if isset($errors) && $errors}
	<div class="alert alert-danger alert-dismissable alert-data-icon" data-icon="r">
            ...
            <div>
                {if $errors|@count > 1}{l s='There are %d errors' sprintf=$errors|@count}{/if}
                {if $errors|@count == 1}
                ...
                {*if isset($smarty.server.HTTP_REFERER) && !strstr($request_uri, 'authentication') && preg_replace('#^https?://[^/]+/#', '/', $smarty.server.HTTP_REFERER) != $request_uri}
                    <div class="lnk"><a href="{$smarty.server.HTTP_REFERER|escape:'html':'UTF-8'|secureReferrer}" title="{l s='Back'}">&laquo; {l s='Back'}</a></div>
                {/if*}
            </div>
	</div>
{/if}