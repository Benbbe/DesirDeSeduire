...
{if isset($products) && $products}
    ...
    {*<ul{if isset($id) && $id} id="{$id}"{/if} class="product_list grid row{if isset($class) && $class} {$class}{/if}">*}
        {foreach from=$products item=product name=products}
            ...
            <div class="ajax_block_product{if $page_name == 'index' || $page_name == 'product'} col-md-3 col-sm-6 pl-item{else} col-md-3 col-sm-6 pl-item{/if}">
                    ...
                    <figure>
                        ...
                        <figcaption>
                            {if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
                                {if ($product.allow_oosp || $product.quantity > 0)}
                                    {if isset($static_token)}
                                        <a href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" data-id-product="{$product.id_product|intval}" class="pl-button pl-addcart ajax_add_to_cart_button" data-toggle="tooltip" data-placement="top" title="{l s='Add to cart'}"><i class="icon-bag"></i></a>
                                    {else}
                                        <a href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" data-id-product="{$product.id_product|intval}" class="pl-button pl-addcart ajax_add_to_cart_button" data-toggle="tooltip" data-placement="top" title="{l s='Add to cart'}"><i class="icon-bag"></i></a>
                                    {/if}
                                {else}
                            ...
                            {if isset($comparator_max_item) && $comparator_max_item}
                                <a href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product}" class="pl-button pl-compare add_to_compare" data-toggle="tooltip" data-placement="top" title="{l s='Add to Compare'}"><i class="arrow_left-right_alt"></i></a>
                            {/if}
                            {hook h='displayProductListFunctionalButtons' product=$product}
                        </figcaption>
                    </figure>
                    <div class="pl-caption" style="min-height: {if !$PS_CATALOG_MODE}150{else}50{/if}px">
                        ...
                        <a class="product-name" href="{$product.link|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}" itemprop="url" >
                            <p class="pl-name">{$product.name|truncate:40:'...'|escape:'html':'UTF-8'}</p>
                        </a>
                        {if (!$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && ((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
                            {if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}
                                <p itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="stock-status">
                                    {if ($product.allow_oosp || $product.quantity > 0)}
            ...
            {elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 0}
