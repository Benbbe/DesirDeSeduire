...
{
	$('#layered_form .s-styled').customSelect();
...
function reloadContent(params_plus)
{
	...
	if (!ajaxLoaderOn)
	{
		...
		$('.products-list>.row').css('opacity', '0.7');
		ajaxLoaderOn = 1;
	}
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			$('.products-list>.row').removeClass('no-gutter').css('opacity', '1');
...
function loadSecondImages(){
    ...
    $.post(
        '/?controller=category&rand=12342414747',
        {
            action: 'loadImages',
            ajax: true,
            token: token,
            ...
            for(i in data){
                var $el = $('[data-product-id="' + i + '"]');
                if ($el.hasClass('plv-item')){
                    $el.find('.product_img_link').append('<span class="plv-backside"></span>');
                    $el.find('.plv-backside').append('<img src="' + data[i] + '" />');
                }else{
                    $el.find('.product_img_link').append('<span class="pl-backside"></span>');
                    $el.find('.pl-backside').append('<img src="' + data[i] + '" />');
                }
            }
        }, 'json'
    );
...
function updateProductUrl()
{
	...
	if (typeof(param_product_url) != 'undefined' && param_product_url != '' && param_product_url !='#') {
		$.each($('div.products-list li.ajax_block_product .product_img_link,'+
				'div.products-list li.ajax_block_product h5 a,'+
				'div.products-list li.ajax_block_product .product_desc a,'+
				'div.products-list li.ajax_block_product .lnk_view'), function() {
