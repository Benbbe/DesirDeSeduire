...
{capture name=pageTitle}{l s='Product Comparison'}{/capture}
<div class="container">
    <div class="row">
        <div class="container">
           <div class="col-xs-12 product-top-line">
              <a class="btn btn-yet-col col-xs-2 back-catalog pull-left upcs" href="{$base_dir}">{l s='Continue Shopping'}</a>
           </div>
        </div>
    </div>
    <div class="row">
		...
		<table id="product_comparison" class="table table-bordered">
			<tr>
				...
				{foreach from=$products item=product name=for_products}
					...
					<td class="ajax_block_product comparison_infos product-block product-{$product->id} products-{$products|@count}">
						<div class="remove">
						...
						<div class="product-image-block">
							...
							{if isset($product->new) && $product->new == 1}
								<a class="new-box" href="{$product->getLink()|escape:'html':'UTF-8'}">
									<span class="pl-badge-new">{l s='New'}</span>
								</a>
							{/if}
							{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
								{if $product->on_sale}
									<a class="sale-box" href="{$product->getLink()|escape:'html':'UTF-8'}">
										<span class="pl-badge-sale">{l s='Sale!'}</span>
									</a>
								{/if}
							{/if}
						...
						<div class="pl-price-block">
							{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
								<span class="pl-price price product-price">{convertPrice price=$product->getPrice($taxes_behavior)}</span>
								...
								<div class="button-container">
									{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
										{if ($product->quantity > 0 OR $product->allow_oosp)}
											<a class="btn btn-sec-col ajax_add_to_cart_button" data-id-product="{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" title="{l s='Add to cart'}">
												<span>{l s='Add to cart'}</span>
											</a>
										{else}
											<span class="btn btn-yet-col ajax_add_to_cart_button button disabled">
												<span>{l s='Add to cart'}</span>
											</span>
										{/if}
									{/if}
									<a class="button lnk_view btn-prim-col btn" href="{$product->getLink()|escape:'html':'UTF-8'}" title="{l s='View'}">
										<span>{l s='View'}</span>
									</a>
								</div>
							</div>
			...
			{if $ordered_features}
				{foreach from=$ordered_features item=feature}
					<tr>
						...
						<td class="{$classname} feature-name td_empty" >
