...
{/if}
<div class="order_carrier_content ovh box">
	...
	{else}
		...
		{else}
			{if $recyclablePackAllowed}
				<div class="">
                                    <input class="stl" type="checkbox" name="recyclable" id="recyclable" value="1" {if $recyclable == 1}checked="checked"{/if} />
                                    <label class="stl" for="recyclable"><span></span>{l s='I would like to receive my order in recycled packaging.'}.</label>
				</div>
			{/if}
			<div class="delivery_options_address">
							...
							{foreach $option_list as $key => $option}
								<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
									<div>
										<table class="resume table table-bordered">
											<tr>
												<td class="delivery_option_radio">
													<input id="delivery_option_{$id_address}_{$option@index}" class="stl delivery_option_radio" type="radio" name="delivery_option[{$id_address}]" data-key="{$key}" data-id_address="{$id_address|intval}" value="{$key}"{if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key} checked="checked"{/if} />
                                                                                                        <label class="stl" for="delivery_option_{$id_address}_{$option@index}"><span><span></span></span></label>
												</td>
										...
										<table class="delivery_option_carrier {if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key}selected{/if} {if $option.unique_carrier}hidden not-displayable{/if}">
											{foreach $option.carrier_list as $carrier}
												<tr>
													{if !$option.unique_carrier}
														<td class="first_item">
				...
				<div id="extra_carrier" style="display: none;"></div>
					{if $giftAllowed}
						<h6 class="upcs">{l s='Gift'}</h6>
						<p class="gift">
                                                        <input class="stl" type="checkbox" name="gift" id="gift" value="1" {if $cart->gift == 1}checked="checked"{/if} />
                                                        <label for="gift" class="stl">
                                                            <span></span>
			...
			{/if}
			{if $opc}
				<h6 class="upcs">{l s='Leave a message'}</h6>
	 			<div>
			...
			{if $conditions AND $cms_id}
				<h6 class="upcs">{l s='Terms of service'}</h6>
				<p class="">
                                        <input class="stl" type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} />
                                        <label class="stl" for="cgv">
                                            <span></span>
		...
		{if !$opc}
					...
					{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
						<button type="submit" name="processCarrier" class="pull-right btn-sec-col btn-md btn btn-default standard-checkout button-medium">
							<span>
								{l s='Proceed to checkout'}
								<i class="arrow_triangle-right right"></i>
							</span>
						</button>
					{/if}
                                    </div>
