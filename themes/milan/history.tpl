        ...
        <div class="{if !$hide_left_column || !$hide_right_column}{if !$hide_left_column && !$hide_right_column}col-md-6{else}col-md-9{/if}{else}col-md-12{/if} {if !$hide_left_column}col-md-push-3{/if}">
            ...
            <div class="row">
                <div class="col-xs-12 col-md-2 left-menu">
                    {hook h="displayMyAccountLeft" active='history'}
                </div>
...
<div class="block-center" id="block-history">
	{if $orders && count($orders)}
		...
		<table id="order-list" class="table resp-tbl table-bordered">
			<thead>
				<tr>
				...
				{foreach from=$orders item=order name=myLoop}
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						<td class="history_link bold">
                                                    <div class="resp-tbl-head">{l s='Order reference'}</div>
						...
						<td class="history_method">
                                                    <div class="resp-tbl-head">{l s='Payment'}</div>
                                                    <div class="resp-tbl-cnt">
						...
						<td class="history_invoice">
                                                    <div class="resp-tbl-head">{l s='Invoice'}</div>
                                                    <div class="resp-tbl-cnt">
						...
						<td class="history_detail">
							...
							<a class="btn btn-default button btn-sm btn-prim-col" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
								<span>
									{l s='Details'}
								</span>
							</a>
							...
							{if isset($opc) && $opc}
								<a class="btn btn-sm btn-yet-col" href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
							{else}
								<a class="btn btn-sm btn-yet-col" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
							{/if}
