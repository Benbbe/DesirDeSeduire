...
*}
<div class="{if !$opc}container{else}{/if}">
...
<div id="opc_new_account" class="opc-main-block">
        ...
        <div class="row">
            <div class="col-sm-6">
                <form action="{$link->getPageLink('authentication', true, NULL, "back=order-opc")|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
                        <fieldset>
                                <h6 class="upcs page-subheading">{l s='Already registered?'}</h6>
                                ...
                                <div id="login_form_content">
                                        ...
                                        <p class="form-group">
                                            <label for="login_email" class="type-text">{l s='Email address'}
                                                <input type="text" class="form-control validate" id="login_email" name="email" data-validate="isEmail" />
                                            </label>
                                        </p>
                                        <p class="form-group">
                                            <label for="login_passwd" class="type-text">{l s='Password'}
            ...
            <div class="col-sm-6">
                <h6 class="upcs page-subheading">{l s='New Customer'}</h6>
                ...
                <p class="opc-button">
                    <button type="submit" class="button  btn btn-third-col btn-sm" id="opc_createAccount">
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html'}" method="post" id="new_account_form" class="std" autocomplete="on" autofill="on">
		<fieldset>
        	<div class="box">
                <h6 id="new_account_title" class="upcs page-subheading">{l s='New Customer'}</h6>
                <div id="opc_account_choice" class="row">
                    <div class="col-xs-12 col-md-12">
                        <p class="title_block">{l s='Instant Checkout'}</p>
                        <p class="opc-button">
                            <button type="submit" class="button  btn btn-third-col btn-sm" id="opc_guestCheckout"><span>{l s='Guest checkout'}</span></button>
                        </p>
                    </div>
                </div>
				...
				<div class="required text form-group">
					<label class="type-text" for="email">{l s='Email'} <sup>*</sup>
				...
				<div class="required password is_customer_param form-group">
					<label class="type-text" for="passwd">{l s='Password'} <sup>*</sup>
				...
				<div class="required clearfix gender-line form-group">
                                        <label class="type-text">{l s='Social title'}</label><br/>
					{foreach from=$genders key=k item=gender}	
                                            <input class="stl" type="radio" name="id_gender" id="id_gender{$gender->id_gender}" value="{$gender->id_gender}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id_gender}checked="checked"{/if} />
                                            <label class="top" for="id_gender{$gender->id_gender}"><span><span></span></span>{$gender->name}</label>
					{/foreach}
				</div>
				...
				<div class="">
                                    <input type="checkbox" class="stl" name="newsletter" id="newsletter" value="1" {if isset($guestInformations) && isset($guestInformations.newsletter) && $guestInformations.newsletter}checked="checked"{/if} autocomplete="off"/>
                                    <label class="stl" for="newsletter"><span></span> {l s='Sign up for our newsletter!'}</label>
				</div>
				<div class="form-group" >
                                        <input type="checkbox" class="stl" name="optin" id="optin" value="1" {if isset($guestInformations) && isset($guestInformations.optin) && $guestInformations.optin}checked="checked"{/if} autocomplete="off"/>
                                        <label class="stl" for="optin"><span></span> {l s='Receive special offers from our partners!'}</label>
                                </div>
				{/if}
				<h6 class="upcs page-subheading top-indent">{l s='Delivery address'}</h6>
				{$stateExist = false}
				{$postCodeExist = false}
				{$dniExist = false}
				...
				<div id="vat_number_block" style="display:none;">
					<div class="form-group">
						<label class="type-text" for="vat_number">{l s='VAT number'}
				...
				<div class="text is_customer_param form-group">
					<label class="type-text" for="address2">{l s='Address (Line 2)'}</label>
				...
				<div class="form-group">
                                        <input type="checkbox" class="stl" name="invoice_address" id="invoice_address"{if (isset($smarty.post.invoice_address) && $smarty.post.invoice_address) || (isset($guestInformations) && isset($guestInformations.invoice_address) && $guestInformations.invoice_address)} checked="checked"{/if} autocomplete="off"/>
                                        <label class="stl" for="invoice_address"><span></span> {l s='Please use another address for invoice'}</label>
				</div>
				...
				<div id="opc_invoice_address" class="is_customer_param">
					...
					<h6 class="upcs page-subheading top-indent">{l s='Invoice address'}</h6>
					{foreach from=$inv_all_fields item=field_name}
					{if $field_name eq "company" && $b2b_enable}
					<div class="form-group">
						<label class="type-text" for="company_invoice">{l s='Company'}
					...
					<div id="vat_number_block_invoice" class="is_customer_param" style="display:none;">
						<div class="form-group">
							<label class="type-text" for="vat_number_invoice">{l s='VAT number'}
					...
					{elseif $field_name eq "firstname"}
					<div class="required form-group">
						<label class="type-text" for="firstname_invoice">{l s='First name'} <sup>*</sup>
					...
					{elseif $field_name eq "lastname"}
					<div class="required form-group">
						<label class="type-text" for="lastname_invoice">{l s='Last name'} <sup>*</sup>
					...
					{elseif $field_name eq "address1"}
					<div class="required form-group">
						<label class="type-text" for="address1_invoice">{l s='Address'} <sup>*</sup>
					...
					{elseif $field_name eq "address2"}
					<div class="form-group is_customer_param">
						<label class="type-text" for="address2_invoice">{l s='Address (Line 2)'}
					...
					<div class="required postcode_invoice form-group">
						<label class="type-text" for="postcode_invoice">{l s='Zip/Postal Code'} <sup>*</sup>
					...
					{elseif $field_name eq "city"}
					<div class="required form-group">
						<label class="type-text" for="city_invoice">{l s='City'} <sup>*</sup>
					...
					{elseif $field_name eq "country" || $field_name eq "Country:name"}
					<div class="required form-group">
						<label class="type-text" for="id_country_invoice">{l s='Country'} <sup>*</sup>
						<select name="id_country_invoice" id="id_country_invoice" class="s-styled form-control">
							<option value="">-</option>
							{foreach from=$countries item=v}
					...
					<div class="required id_state_invoice form-group" style="display:none;">
						<label class="type-text" for="id_state_invoice">{l s='State'} <sup>*</sup>
						<select name="id_state_invoice" id="id_state_invoice" class="s-styled form-control">
							<option value="">-</option>
						</select>
                                                </label>
					</div>
					...
					{/if}
					<div class="form-group is_customer_param">
						<label class="type-text" for="other_invoice">{l s='Additional information'}
					...
					{/if}					
					<div class="form-group is_customer_param">
						<label class="type-text" for="phone_invoice">{l s='Home phone'}
					...
					<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
						<label class="type-text" for="phone_mobile_invoice">{l s='Mobile phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}
				...
				<div class="submit opc-add-save clearfix">
                    ...
                    <button type="submit" name="submitAccount" id="submitAccount" class="button  btn btn-third-col btn-sm"><span>{l s='Save'}</span></button>
