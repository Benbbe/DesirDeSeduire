...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
                        <p class="dark pull-left">
                            <strong>{l s='Order Reference %s - placed on' sprintf=$order->getUniqReference()} {dateFormat date=$order->date_add full=0}</strong>
			</p>
                        <a href="#" onclick="$(this).closest('form').submit(); return false;" class="pull-right"><i class="icon-refresh"></i> <span>{l s='Reorder'}</span></a>
...
<div class="table_block table-container">
	<table class="detail_step_by_step resp-tbl table table-bordered">
		<thead>
			<tr>
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				<td class="step-by-step-date">
                                    <div class="resp-tbl-head">{l s='Date'}</div>
                                    <div class="resp-tbl-cnt">
                                        {dateFormat date=$state.date_add full=0}
                                    </div>
                                </td>
				<td>
                                    <div class="resp-tbl-head">{l s='Status'}</div>
                                    <div class="resp-tbl-cnt">
...
<div class="adresses_block">
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			...
			<ul class="address list-unstyled alternate_item box">
...
<div class="table_block table-container">
	<table class="table table-bordered resp-tbl">
		<thead>
			<tr>
		...
		<tfoot>
			{if $priceDisplay && $use_tax}
				<tr class="item">
                                    <td colspan="{if $return_allowed}4{else}3{/if}" class="hidden-xs">
						<strong>{l s='Items (tax excl.)'}</strong>
					</td>
					<td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                            <div class="resp-tbl-head">{l s='Items (tax excl.)'}</div>
                                            <div class="resp-tbl-cnt">
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                    <div class="resp-tbl-head">{l s='Items'} {if $use_tax}{l s='(tax incl.)'}{/if}</div>
                                            <div class="resp-tbl-cnt">
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                    <div class="resp-tbl-head">{l s='Total vouchers'}</div>
                                            <div class="resp-tbl-cnt">
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                    <div class="resp-tbl-head">{l s='Total gift wrapping cost'}</div>
                                            <div class="resp-tbl-cnt">
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                    <div class="resp-tbl-head">{l s='Shipping & handling'} {if $use_tax}{l s='(tax incl.)'}{/if}</div>
                                            <div class="resp-tbl-cnt">
			...
			<tr class="totalprice item">
				<td colspan="{if $return_allowed}4{else}3{/if}" class="hidden-xs">
					<strong>{l s='Total'}</strong>
                                ...
                                <td colspan="{if $order->hasProductReturned()}3{else}2{/if}" class="text-right">
                                    <div class="resp-tbl-head">{l s='Total'}</div>
                                            <div class="resp-tbl-cnt">
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td>
                                                    <div class="resp-tbl-head">{l s='Reference'}</div>
                                                    <div class="resp-tbl-cnt">
						...
						<td class="bold">
                                                    <div class="resp-tbl-head">{l s='Product'}</div>
                                                    <div class="resp-tbl-cnt">
                                                        <label for="cb_{$product.id_order_detail|intval}">{$product.product_name|escape:'html':'UTF-8'}</label>
                                                    </div>
						</td>
						<td>
                                                    <div class="resp-tbl-head">{l s='Quantity'}</div>
                                                    <div class="resp-tbl-cnt">
                                                        <input type="hidden" name="order_qte_input[{$smarty.foreach.products.index}]" value="{$product.customizationQuantityTotal|intval}" class="cart_quantity_input count form-control grey" autocomplete="off" size="2">
                                                        <span>{$product.customizationQuantityTotal|intval}</span>
                                                    </div>
                                                </td>
						{if $order->hasProductReturned()}
							<td>
                                                            <div class="resp-tbl-head">{l s='Returned'}</div>
                                                            <div class="resp-tbl-cnt">
						...
						{/if}
						<td>
                                                    <div class="resp-tbl-head">{l s='Unit price'}</div>
                                                    <div class="resp-tbl-cnt">
						...
						<td>
                                                    <div class="resp-tbl-head">{l s='Total price'}</div>
                                                    <div class="resp-tbl-cnt">
							<label class="price" for="cb_{$product.id_order_detail|intval}">
					...
					{foreach $product.customizedDatas  as $customizationPerAddress}
						...
						<tr class="alternate_item">
							...
							<td>
                                                            <input type="text" name="customization_qty_input[{$customizationId|intval}]" value="{$customization.quantity|intval}" class="cart_quantity_input count form-control grey" autocomplete="off" size="2">
                                                            <span>{$customization.quantity|intval}</span>
							</td>
							<td colspan="2"></td>
						</tr>
						{/foreach}
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="return_quantity">
                                                    <div class="resp-tbl-head">{l s='Quantity'}</div>
                                                        <div class="resp-tbl-cnt">
                                                            <input type="hidden" name="order_qte_input[{$product.id_order_detail|intval}]" value="{$productQuantity|intval}" class="cart_quantity_input count form-control grey" autocomplete="off" size="2">
                                                            <span>{$productQuantity|intval}</span>
                                                        </div>
                                                </td>
						{if $order->hasProductReturned()}
							<td>
...
{if $order->getShipping()|count > 0}
	...
	<table class="table table-bordered footab resp-tbl">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
		...
		<tbody>
			...
			<tr class="item">
				<td data-value="{$line.date_add|regex_replace:"/[\-\:\ ]/":""}">
                                    <div class="resp-tbl-head">{l s='Date'}</div>
                                    <div class="resp-tbl-cnt">
                                    {dateFormat date=$line.date_add full=0}
                                    </div>
                                </td>
				<td>
                                    <div class="resp-tbl-head">{l s='Carrier'}</div>
                                    <div class="resp-tbl-cnt">
                                    {$line.carrier_name}
                                    </div>
                                </td>
				<td data-value="{if $line.weight > 0}{$line.weight|string_format:"%.3f"}{else}0{/if}">
                                    <div class="resp-tbl-head">{l s='Weight'}</div>
                                    <div class="resp-tbl-cnt">
                                    {if $line.weight > 0}{$line.weight|string_format:"%.3f"} {Configuration::get('PS_WEIGHT_UNIT')}{else}-{/if}
                                    </div>
                                </td>
				<td data-value="{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{$line.shipping_cost_tax_incl}{else}{$line.shipping_cost_tax_excl}{/if}">
                                    <div class="resp-tbl-head">{l s='Shipping cost'}</div>
                                    <div class="resp-tbl-cnt">
				...
				<td>
                                    <div class="resp-tbl-head">{l s='Tracking number'}</div>
                                    <div class="resp-tbl-cnt">
...
{if !$is_guest}
	...
	{if isset($errors) && $errors}
		<div class="alert alert-danger alert-dismissable alert-data-icon" data-icon="r">
                    ...
                    <div>
                        <p><b>{if $errors|@count > 1}{l s='There are %d errors' sprintf=$errors|@count}{else}{l s='There is %d error' sprintf=$errors|@count}{/if}</b></p>
			<ol>
	...
	<div class="alert alert-success alert-dismissable alert-data-icon" data-icon="R">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            <div>
	...
	{/if}
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std ovh" id="sendOrderMessage">
		<h3 class="page-heading bottom-indent">{l s='Add a message'}</h3>
		<p>{l s='If you would like to add a comment about your order, please write it in the field below.'}</p>
		<p class="form-group">
                    <label for="id_product" class="type-text">{l s='Product'}
			<select name="id_product" class="s-styled form-control">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="btn btn-third-col pull-right btn-sm"><span>{l s='Send'}<i class="icon-chevron-right right"></i></span></button>
		</div>
	</form>
