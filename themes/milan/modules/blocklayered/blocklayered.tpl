			...
			<div>
				{if isset($selected_filters) && $n_filters > 0}
                                    <div class="widget wg-active-fiters" id="enabled_filters">
                                        <h3 class="wg-title">{l s='Enabled filters:' mod='blocklayered'}</h3>
                                        <div class="wg-body">
                                           <ul class="wg-af-list">
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
                                                                                                    <a class="wg-afl-del" href="#" rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}"><i class="icon_close"></i></a>
													{if $filter.format == 1}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
                                                                                                            <a href="#" class="wg-afl-del" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}">
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
                        ...
                        <div class="widget wg-filterslist layered_subtitle_heading">
                            <h3 class="wg-title">{$filter.name|escape:'html':'UTF-8'}</h3>
						
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="wgfl-list layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
										<li class="{if isset($filter.is_color_group) && $filter.is_color_group} col-sm-6 {/if} {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
											...
											{else}
												<input type="checkbox" class="checkbox stl" name="layered_{$filter.type_lite}_{$id_value}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}{if !$value.nbr} disabled="disabled"{/if} /> 
											{/if}
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="stl disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{else} class="stl"{/if}{/if}>
                                                                                            <span></span>
                                                                                            {if !$value.nbr}
                                                                                            {$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
                                                                                            {else}
                                                                                                <a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<em class="wgfl-count"> ({$value.nbr})</em>{/if}</a>
                                                                                            {/if}
											</label>
										</li>
										{/if}
								...
								{else}
									...
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
                                                                                    ...
                                                                                    {else}
                                                                                            <input type="radio" class="stl radio layered_{$filter.type_lite}_{$id_value}" name="layered_{$filter.type_lite}{if $filter.id_key}_{$filter.id_key}{else}_1{/if}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}{/if}" value="{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}{if !$value.nbr} disabled="disabled"{/if} /> 
                                                                                            <label for="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}{/if}"{if !$value.nbr} class="stl disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                                                                                    <span><span></span></span>    
                                                                                                    {if !$value.nbr}
                                                                                                            {$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
                                                                                                    {else}
									...
									{/foreach}
									{else}
										<select class="s-styled select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
							...
							{else}
								{if $filter.filter_type == 0}
                                                                    ...
                                                                    <span id="layered_{$filter.type}_range" class="price-range-label" data-currency-sign="$" data-cursign-before="true"></span>
                                                                    </p>
                                                                    </li>
								{else}
									{if $filter.filter_type == 1}
