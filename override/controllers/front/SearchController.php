        ...
        if ($this->ajax_search) {
            ...
            foreach ($searchResults as &$product) {
                ...
                if (isset($imageID['id_image'])) {
                    $imgLink = $this->context->link->getImageLink(
                        $product['prewrite'],
                        (int)$product['id_product'].'-'.$imageID['id_image']
...
configuration tools. */
                ...
                $product['product_price'] = Product::getPriceStatic(
                ...
                if ($taxes == 0 or $taxes == 2) {
                    $product['product_price'] = Tools::displayPrice(
        ...
        if ($this->instant_search && !is_array($query)) {
            ...
            if (method_exists(
                ...
                for ($i = 0; $i < $nbProducts; $i++) {
                    if (isset($search['result'][$i]['id_product'])) {
                        $image_array[$search['result'][$i]['id_product']] = Product::getSecondImage(
                            $search['result'][$i]['id_product']
                        );
                    }
                }
            ...
            $nbProducts = (int)(Search::searchTag(
                $this->context->language->id,
                $tag,
                true
            ));
            $this->pagination($nbProducts);
            $result = Search::searchTag(
                $this->context->language->id,
                $tag,
                false,
                $this->p,
            ...
            if (method_exists(
                ...
                for ($i = 0; $i < $nbProducts; $i++) {
                    if (isset($result[$i]['id_product'])) {
                        $image_array[$result[$i]['id_product']] = Product::getSecondImage($result[$i]['id_product']);
                    }
                }
                $this->context->smarty->assign(
                    'productimg',
