...
class WishListAzl extends ObjectModel
{
    /**
    ...
    public static function refreshWishList($id_wishlist)
    {
        ...
        $freshwish = Db::getInstance()->executeS(
            ...
            WHERE (wp.id_wishlist = '.(int)($id_wishlist).' AND ((cp.id_product IS NULL AND cp.id_product_attribute IS
             NULL)))
            '
        );
        $res = Db::getInstance()->executeS(
            ...
            JOIN `'._DB_PREFIX_.'wishlist_product` wp ON (wp.id_wishlist_product = wpc.id_wishlist_product)
            JOIN `'._DB_PREFIX_.'cart` c ON (c.id_cart = wpc.id_cart)
            JOIN `'._DB_PREFIX_.'cart_product` cp ON (cp.id_cart = wpc.id_cart AND cp.id_product = wp.id_product AND
            cp.id_product_attribute = wp.id_product_attribute)
            ...
            foreach ($res as $refresh) {
                if ($refresh['wish_quantity'] > $refresh['cart_quantity']) {
                    Db::getInstance()->execute(
                        ...
                        SET `quantity`= `quantity` + '.((int)($refresh['wish_quantity'])
                            - (int)($refresh['cart_quantity'])).'
    ...
    public static function getProductByIdCustomer(
        ...
        $products = Db::getInstance()->executeS(
            '
        SELECT w.`name` as wishname, wp.`id_product`, wp.`quantity`, p.`quantity` AS product_quantity, pl.`name`, wp
        .`id_product_attribute`, wp.`priority`, pl.link_rewrite, cl.link_rewrite AS category_rewrite
        ...
        LEFT JOIN `'._DB_PREFIX_
            .'category_lang` cl ON cl.`id_category` = product_shop.`id_category_default` AND cl.id_lang='.(int)$id_lang
            .Shop::addSqlRestrictionOnLang('cl').'
        WHERE w.`id_customer` = '.(int)($id_customer).'
        AND pl.`id_lang` = '.(int)($id_lang).(empty($id_product) === false ?
                ' AND wp.`id_product` = '.(int)($id_product) : '').($quantity == true ? ' AND wp.`quantity` != 0' :
                '').'
        ...
        for ($i = 0; $i < sizeof($products); ++$i) {
            if (isset($products[$i]['id_product_attribute']) and
                ...
                LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang`
                 = '.(int)($id_lang).')
                LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl ON (ag.`id_attribute_group` = agl
                .`id_attribute_group` AND agl.`id_lang` = '.(int)($id_lang).')
                LEFT JOIN `'._DB_PREFIX_.'product_attribute` pa ON (pac.`id_product_attribute` = pa
                ...
                $products[$i]['attributes_small'] = rtrim(
                    $products[$i]['attributes_small'],
                    ', '
                );
                if (isset($result[0])) {
    ...
    public static function addProduct(
        ...
        if (!Validate::isUnsignedId($id_wishlist) or !Validate::isUnsignedId($id_customer) or
            !Validate::isUnsignedId($id_product) or !Validate::isUnsignedId($quantity)
        ) {
            die (Tools::displayError());
        }
        $result = Db::getInstance()->getRow(
    ...
    public static function updateProduct(
        ...
        if (!Validate::isUnsignedId($id_wishlist) or !Validate::isUnsignedId($id_product) or
            !Validate::isUnsignedId($quantity) or $priority < 0 or $priority > 2) {
            die (Tools::displayError());
        }
     ...
     * @return boolean succeed
     */
    public static function addBoughtProduct(
        $id_wishlist,
        $id_product,
        $id_product_attribute,
        $id_cart,
        ...
        if (!Validate::isUnsignedId($id_wishlist) or !Validate::isUnsignedId($id_product)
        ...
        if (!sizeof($result) or ($result['quantity'] - $quantity) < 0 or $quantity > $result['quantity']) {
            return (false);
        }
