...
class HompageTabs extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->description
            = $this->l('Displays Featured, special, new and category,  Products in the middle of your homepage.');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitModule')) {
            ...
            if (!(is_array($items) && Tools::getValue('showcategory') && count($items)
                && Configuration::updateValue(
                    'CATEGORY_TABS_ID',
                    (string)implode(
                        ',',
            ...
            Configuration::updateValue(
                'PS_tab_sliderenabled',
                (int)(Tools::getValue('sliderenabled'))
            );
            Configuration::updateValue(
                'PS_tab_showfeatured',
                (int)(Tools::getValue('showfeatured'))
            );
            Configuration::updateValue(
                'PS_tab_shownew',
                (int)(Tools::getValue('shownew'))
            );
            Configuration::updateValue(
                'PS_tab_showspecial',
                (int)(Tools::getValue('showspecial'))
            );
            Configuration::updateValue(
                'PS_tab_showcategory',
                (int)(Tools::getValue('showcategory'))
            );
    ...
    public function renderForm()
    {
        ...
        if (Shop::isFeatureActive()) {
            $fields_form['form']['description']
                = $this->l('The modifications will be applied to').' '.(Shop::getContext() == Shop::CONTEXT_SHOP ?
                    $this->l('shop').' '.$this->context->shop->name : $this->l('all shops'));
        }
    ...
    public function getConfigFieldsValues()
    {
        return array(
            'nbr' => Tools::getValue(
                'HOME_TABS_NBR',
                Configuration::get('HOME_TABS_NBR')
            ),
            'sliderenabled' => Tools::getValue(
                'PS_tab_sliderenabled',
                Configuration::get('PS_tab_sliderenabled')
            ),
            'showfeatured' => Tools::getValue(
                'PS_tab_showfeatured',
                Configuration::get('PS_tab_showfeatured')
            ),
            'shownew' => Tools::getValue(
                'PS_tab_shownew',
                Configuration::get('PS_tab_shownew')
            ),
            'showspecial' => Tools::getValue(
                'PS_tab_showspecial',
                Configuration::get('PS_tab_showspecial')
            ),
            'showcategory' => Tools::getValue(
                'PS_tab_showcategory',
                Configuration::get('PS_tab_showcategory')
            ),
        );
    }
    ...
    private function getMenuItems()
    {
        ...
        if (Tools::strlen($conf)) {
            return explode(
                ',',
                Configuration::get('CATEGORY_TABS_ID')
            );
        } else {
            return array();
        }
    ...
    private function generateCategoriesOption(
        $categories,
        $items_to_skip = null
    ) {
        $html = '';
        ...
        foreach ($categories as $category) {
            if (isset($items_to_skip)
                && !in_array(
                    'CAT'.(int)$category['id_category'],
                    $items_to_skip
                )
            ) {
                $shop = (object)Shop::getShop((int)$category['id_shop']);
                $html
                    .= '<option value="'.(int)$category['id_category'].'">'.str_repeat(
                        '&nbsp;',
                        $this->spacer_size * (int)$category['level_depth']
    ...
    public function hookDisplayHome($params)
    {
        ...
        if (
            (!$this->isCached(
                'hompagetabs.tpl',
                $this->getCacheId('hompagetabs')
            ))
            || (!$this->isCached(
                'hompagetabs-slider.tpl',
                $this->getCacheId('hompagetabs')
            ))
        ) {
            ...
            if (Configuration::get('PS_tab_showfeatured') == 1) {
                ...
                $fproducts = $fcategory->getProducts(
                    $id_lang,
                    1,
                    ($nb ? $nb : 10)
                );
                ...
                if (method_exists(
                    ...
                    for ($i = 0; $i < count($fproducts); $i++) {
                        if (isset($fproducts[$i]['id_product'])) {
                            $image_array[$fproducts[$i]['id_product']]
                                = Product::getSecondImage($fproducts[$i]['id_product']);
                        }
                    }

                    $this->smarty->assign(
                        'fproducts_productimg',
            ...
            if (Configuration::get('PS_tab_showspecial') == 1) {
                if (!Configuration::get('PS_CATALOG_MODE')) {
                    if ($sproducts = Product::getPricesDrop(
                        $id_lang,
                        0,
                        $nb
                    )
                        ...
                        if (method_exists(
                            ...
                            for ($i = 0; $i < count($sproducts); $i++) {
                                if (isset($sproducts[$i]['id_product'])) {
                                    $image_array[$sproducts[$i]['id_product']]
                                        = Product::getSecondImage($sproducts[$i]['id_product']);
                                }
                            }

                            $this->smarty->assign(
                                'sproducts_productimg',
                                (isset($image_array) and $image_array) ? $image_array : null
                            );
                        }
            ...
            if (Configuration::get('PS_tab_shownew') == 1) {
                $nproducts = Product::getNewProducts(
                    $id_lang,
                    0,
                    $nb
                );
                if (method_exists(
                    ...
                    for ($i = 0; $i < count($nproducts); $i++) {
                        if (isset($nproducts[$i]['id_product'])) {
                            $image_array[$nproducts[$i]['id_product']]
                                = Product::getSecondImage($nproducts[$i]['id_product']);
                        }
                    }

                    $this->smarty->assign(
                        'nproducts_productimg',
                        (isset($image_array) and $image_array) ? $image_array : null
                    );
                }
            ...
            if (Configuration::get('PS_tab_showcategory') == 1) {
                ...
                foreach ($menu_item as $item) {
                    ...
                    if (Validate::isLoadedObject($category)) {
                        ...
                        $categories[$item]['products'] = $category->getProducts(
                            $id_lang,
                            1,
                            ($nb ? $nb : 10)
                        );
                        if (method_exists(
                            ...
                            for ($i = 0; $i < count($categories[$item]['products']); $i++) {
                                if (isset($categories[$item]['products'][$i]['id_product'])) {
                                    $image_array[$categories[$item]['products'][$i]['id_product']]
                                        = Product::getSecondImage($categories[$item]['products'][$i]['id_product']);
                                }
                            }

                            $categories[$item]['productimg'] = (isset($image_array) and $image_array) ? $image_array :
                                null;
                        }
                    }
                }
        ...
        if ((Configuration::get('PS_tab_sliderenabled')) == 1) {
            return $this->display(
                __FILE__,
                'views/templates/hook/hompagetabs-slider.tpl',
                $this->getCacheId('hompagetabs')
            );
        } else {
            return $this->display(
                __FILE__,
                'views/templates/hook/hompagetabs.tpl',
                $this->getCacheId(
                    'hompagetabs'
                )
            );
